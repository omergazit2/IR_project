...
class MultiFileReader:
    ...
    def __init__(self, path_to_dir:str):
        """
        path_to_dir format: xxxxx/xxxxx/xxxx/
        """
        self._open_files = {}
    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                ...
                bucket = storage_client.bucket("indexes_wikipidia_ir_project")
                blob = bucket.blob(self.path_to_dir + f_name)
...
class InvertedIndex:  
    def __init__(self, docs={}):
        ...
        # normalization factor write (len docs)
        self.NF = defaultdict(list)
    ...
    def posting_lists_iter(self, path_to_posting_locs):
        """ A generator that reads one posting list from disk and yields 
            a (word:str, [(doc_id:int, tf:int), ...]) tuple.
            parameters:
            -------------
            path_to_posting_locs: the mechin path to indexe's posting location
        """
        with closing(MultiFileReader(path_to_posting_locs)) as reader:
    ...
    @staticmethod
    def read_index(blob_name, bucket_name = "indexes_wikipidia_ir_project"):
        storage_client = storage.Client()
